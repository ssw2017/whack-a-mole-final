...
function whack(event) {
  updateScore();
  hammerCursor.src = "assets/hammer-toy2.png";
  setTimeout(() => {hammerCursor.src = "assets/hammer-toy1.png";}, 250);
  return points;
}
...
function setEventListeners(){
  moles.forEach((mole) => {
    ...
    mole.addEventListener("click", playHammerSound);
  });
  return moles;
}
...
function startGame() {
  points = 0;
  setDuration(Math.trunc(setDelay(difficulty)/100));
  startTimer();
  setEventListeners();
  showUp();
  return "game started";
